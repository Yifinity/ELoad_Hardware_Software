...
void loop() {
  ...
  if(Serial.available()){
    input_message = Serial.readString();
    input_message.trim();
    if(input_message == "BEGIN_TEST"){
      // Serial.println("ON!");
      digitalWrite(LED_BUILTIN, HIGH);
    }else if(input_message == "STOP_TEST"){
      // Serial.println("OFF!");
      digitalWrite(LED_BUILTIN, LOW);
    }
    // }else{
    //   Serial.print("Received:"); Serial.print(input_message); Serial.println(", which has no purpose");
    // }
  }
